    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Comentário feito em
                            <span>
                                @if($comment->actor)
                                ...
                                @if($comment->studio)
                                    ...
                                    <a class="ml-2" href="{{ route('studio.show', [$comment->studio->id, $comment->studio->slug]) }}" target="_blank">{{ $comment->studio->name }}</a>
                                @endif
                                @if($comment->torrent)
                            ...
                            <div class="media-body ml-3">
                                ...
                                <div class="clearfix mt-4">
                                    ...
                                    @if($comment->user_id == auth()->user()->id || auth()->user()->can('painel-staff'))
                                        <a href="{{ route('comments.edit', [$comment->id]) }}" data-toggle="tooltip" title="Editar Comentário">
                                    ...
                                    @if($comment->user_id == auth()->user()->id || auth()->user()->can('painel-staff'))
                                        <a href="javascript:;" onclick="document.getElementById('comment-del-{{ $comment->id }}').submit();">
                                            <i class="fas fa-times text-danger m-l-15" data-toggle="tooltip" title="Deletar Comentário"></i>
                                        </a>
