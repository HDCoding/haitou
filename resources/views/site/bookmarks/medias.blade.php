...
@section('content')
    ...
    <h4 class="font-weight-bold py-3 mb-2">Mídias Favoritas</h4>

    <div class="card">
        <div class="card-header">Mídias Favoritas</div>
        <table class="table card-table">
            ...
            @foreach($bookmarks as $bookmark)
                <tr>
                    <td>
                        <img class="" src="{{ $bookmark->media->poster }}" alt="{{ $bookmark->media->name }}"
                             width="70px">
                    </td>
                    <td>
                        <a href="{{ route('media.show', [$bookmark->media->id, $bookmark->media->slug]) }}"
                           target="_blank">{{ $bookmark->media->name }}</a>
                    </td>
